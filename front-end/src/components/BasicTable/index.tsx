...
function BasicTable<T>({
  ...
  const THeadList = useMemo(() => {
    return columnDefs.map((item) => (
      <Col key={`THead-${item.id}`} width={item.width} align={item.align || 'center'}>
        {item.name}
      </Col>
    ));
  }, [columnDefs]);
  ...
  const TBodyList = useMemo(() => {
    ...
    return rowData.map((rowItem: any) => (
      <Div key={`TBody-${rowItem[uniqueKey]}`}>
        <Row isActive={seletedVal === rowItem[uniqueKey]} onClick={() => handleRowClick(rowItem)}>
          {columnDefs.map(({ id, width, align, parser }) => (
            <Col key={`TBody-Col-${id}`} width={width} align={align}>
  ...
  return (
    <TableWrap id={tableId}>
      <THead id={`${tableId}_head`}>{THeadList}</THead>
      <TBody id={`${tableId}_body`}>{TBodyList || '데이터가 존재하지 않습니다.'}</TBody>
    </TableWrap>
  );
}
